...
export const handler = async (event, context) => {
  ...
  // worker aways execute write sync operation
  ...
  const processEvent = async (data, adapterInstance) => {
    switch (data.operation) {
      case 'CREATE':
        return adapterInstance.opening.createOpening(data.payload)
      case 'UPDATE':
        return adapterInstance.opening.updateOpening(data.payload.id, data.payload.talentEconomicSegment, data.payload)
      case 'DELETE':
        return adapterInstance.opening.deleteOpening(data.payload.id, data.payload.talentEconomicSegment)
      case 'MATCH':
        // the match process occurs in opost repository
        return adapterInstance.talent.matchTalentsFromOpening(data.payload)
      default:
        // the match process occurs in opost repository
        return adapterInstance.talent.matchTalentsFromOpening(data.payload)
    }
